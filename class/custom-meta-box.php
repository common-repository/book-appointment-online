...
class AT_Meta_Box {
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($typenow,$this->_meta_box['pages']) && $this->is_edit_page()){
      ...
      wp_enqueue_style( 'at-meta-box', plugins_url( '/css/meta-box.css', dirname(__FILE__) ) );
      
      // Enqueue Meta Box Scripts
      wp_enqueue_script( 'at-meta-box', plugins_url( '/js/meta-box.js', dirname(__FILE__) ), array( 'jquery' ), null, true );
  ...
  public function check_field_select() {
    ...
    if ( ! $this->has_field( 'select' ) && ! $this->has_field( 'posts' ))
      ...
      wp_enqueue_style('at-multiselect-select2-css', plugins_url( '/js/select2/select2.css', dirname(__FILE__) ), array(), null);
      wp_enqueue_script('at-multiselect-select2-js', plugins_url( '/js/select2/select2.js', dirname(__FILE__) ), array('jquery'), false, true);
  }
  ...
  public function check_field_date() {
    ...
    if ( $this->has_field( 'date' ) && $this->is_edit_page() ) {
      // Enqueu JQuery UI, use proper version.
	  $plugin_path = $this->SelfPath;
      wp_enqueue_style( 'at-jquery-ui-css', plugins_url( '/js/jquery-ui/jquery-ui.css', dirname(__FILE__) ) );
  ...
  public function check_field_time() {
    ...
    if ( $this->is_edit_page() ) {
      ...
      // Enqueu JQuery UI, use proper version.
      wp_enqueue_style( 'at-jquery-ui-css', plugins_url( '/js/jquery-ui/jquery-ui.css', dirname(__FILE__) ) );
      wp_enqueue_script( array('jquery-ui','jquery-ui-datepicker'));
	  if (!wp_script_is('jquery-ui-datepicker')) wp_enqueue_script( 'jquery-ui-datepicker');
      wp_enqueue_script( 'at-timepicker', plugins_url( '/js/jquery-ui/jquery-ui-timepicker-addon.js', dirname(__FILE__) ), array(  'jquery' ),false,true );
    }
  }
  ...
  public function check_field_code() {
    ...
    if ( $this->has_field( 'code' ) && $this->is_edit_page() ) {
      ...
      wp_enqueue_style( 'at-code-css', plugins_url( '/js/codemirror/codemirror.css', dirname(__FILE__) ),array(),null);
      wp_enqueue_style( 'at-code-css-dark', plugins_url( '/js/codemirror/solarizedDark.css', dirname(__FILE__) ),array(),null);
      wp_enqueue_style( 'at-code-css-light', plugins_url( '/js/codemirror/solarizedLight.css', dirname(__FILE__) ),array(),null);
      wp_enqueue_script('at-code-js',plugins_url( '/js/codemirror/codemirror.js', dirname(__FILE__) ),array('jquery'),false,true);
      wp_enqueue_script('at-code-js-xml',plugins_url( '/js/codemirror/xml.js', dirname(__FILE__) ),array('jquery'),false,true);
      wp_enqueue_script('at-code-js-javascript',plugins_url( '/js/codemirror/javascript.js', dirname(__FILE__) ),array('jquery'),false,true);
      wp_enqueue_script('at-code-js-css',plugins_url( '/js/codemirror/css.js', dirname(__FILE__) ),array('jquery'),false,true);
      wp_enqueue_script('at-code-js-clike',plugins_url( '/js/codemirror/clike.js', dirname(__FILE__) ),array('jquery'),false,true);
      wp_enqueue_script('at-code-js-php',plugins_url( '/js/codemirror/php.js', dirname(__FILE__) ),array('jquery'),false,true);
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_begin( $field, $meta) {
    echo "<td class='at-field ".(($this->inGroup === true)? " width-50' valign='top'": "' colspan='2'").">";
  ...
  public function show_field_posts($field, $meta) {
    ...
    // select
      ...
      foreach ($posts as $p) {
        ...
        echo '<option value="'.$p->ID.'"' . selected(in_array($p->ID, $meta), true, false).' '.$dataDay.' '.$dataDayUsl.' '.$timeUslugi.'>'.$p->post_title.'</option>';
      }
      echo "</select>";
    }
